...
def save_images_log_power_spectogram(file_path: Path):
    ...
    melspectrogram = feature.melspectrogram(y=y, sr=sr,n_fft=512, n_mels=128, hop_length=128)
    imgdb = display.specshow(power_to_db(melspectrogram, ref=np.max), sr=sr, y_axis='mel', x_axis='time', ax=ax, cmap='gray_r')
    ax.set(title='Log-Power spectrogram')
    fig.colorbar(imgdb, ax=ax, format="%+2.0f dB")
    image_path = Path(image_folder) / f"{path.basename(file_path)}_log_power_spectrogram.png"
