...
from koregraph.api.machine_learning.chore_workflow import (
    generate_and_export_choreography,
)
from koregraph.api.preprocessing.music_to_numpy import music_to_numpy
from koregraph.api.preprocessing.music_to_numpy_interpolated import (
    music_to_numpy_interpolated,
)
...
def generate_pickle_files_interpolated():
    """Generate pickle files for all advanced moves with interpolated audio and posture arrays.
    NOT WORKING!
    """
    for move_name in ALL_ADVANCED_MOVE_NAMES:
        try:
            ...
            posture_array = generate_and_export_choreography(pkl_file)

            # Convert audio file to numpy array
            audio_array = music_to_numpy_interpolated(audio_file)
            ...
            with open(
                GENERATED_PICKLE_DIRECTORY / ("interpolated_" + move_name), "wb"
            ) as f:
...
if __name__ == "__main__":
    generate_pickle_files_interpolated()
